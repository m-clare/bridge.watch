...
def main(fp):
    ...
    df.LONG_017.replace(
        r"(\d{2})\.+(\d*)", "0" + r"\1" + r"\2", inplace=True, regex=True
    )
    df.LAT_016.replace(
        r"(\d{2})\.+(\d*)", "0" + r"\1" + r"\2", inplace=True, regex=True
    )
    ...
    df.loc[(~null_lat), "LAT_016"] = df.loc[~null_lat, "LAT_016"].str.ljust(
        8, fillchar="0"
    )
    ...
    df.to_csv("2024AllRecordsDelimitedAllStatesClean.csv", na_rep="NULL", index=False)
